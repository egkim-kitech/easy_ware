...
class PumpNode:
    def __init__(self):
        ...
        self.odom_sub = rospy.Subscriber("/driving_course", Odometry, self.odom_callback)
        ...
        self.route_points = []  # To store interpolated route points
        self.min_distances = []  # List to store min_distance values
    ...
    def goalcallback(self, msg):
        ...
        if route_x and route_y:
            ...
            route_px, route_py = self.route_interpolation(route_x, route_y, 0.8)  # 0.3m interval
            self.route_points = list(zip(route_px, route_py))
    ...
    def odom_callback(self, msg):
        """Callback function for /odom topic to find closest point."""
        ...
        for px, py in self.route_points:
            distance = math.sqrt((odom_x - px) ** 2 + (odom_y - py) ** 2)
            if distance < min_distance:
                min_distance = distance
                closest_point = (px, py)

        if closest_point:
            print(f"Closest point: {closest_point}, Distance: {min_distance:.2f} meters")
            self.min_distances.append(min_distance)

    def plot_min_distances(self):
        plt.hist(self.min_distances, bins=30, edgecolor='black', color='blue', range=[0, 5])
        plt.xlim(0, 5)
        plt.xlabel('Distance (meters)')
        plt.ylabel('frequency')
        plt.title('Min Distance to Route Points Over Time')
